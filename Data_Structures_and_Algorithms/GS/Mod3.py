...
def status_operacional(leitura):
    if leitura["comunicacao"] == 0:
        return "CRÍTICO"
    elif leitura["temperatura"] > 80 or leitura["energia"] < 20:
        return "ATENÇÃO"
    else:
        return "NOMINAL"
...
def visualizar_status():
    ...
    status = leitura.get("status_operacional", status_operacional(leitura))
    if status == "CRÍTICO":
        print(cor(f"Status Operacional: {status}", "31"))
    elif status == "ATENÇÃO":
        print(cor(f"Status Operacional: {status}", "33"))
    else:
        print(cor(f"Status Operacional: {status}", "32"))
...
def mostrar_historico():
    ...
    for i, leitura in enumerate(historico, start=1):
        ...
        print(f"Energia: {leitura['energia']}%")
        print(f"Comunicação: {'OK' if leitura['comunicacao'] == 1 else 'FALHA'}")
        print(f"Status Operacional: {leitura.get('status_operacional', status_operacional(leitura))}")
